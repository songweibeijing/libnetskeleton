...
int generic_aton(char *name, struct sockaddr_storage *addr)
{
	...
	if (debuglevel) {
		printf("generic_aton(%s, %p)", name, addr);
	}
	...
	if (n != 0) {
		printf("getaddrinfo: %s", gai_strerror(n));
		return 0;
	}
	if (debuglevel) {
		...
		printf("sockaddr = %p", ai->ai_addr);
		printf("canonname = %s", ai->ai_canonname);
	}
	...
	case AF_INET6:
		...
		break;
	default:
		printf("Unknown family %d", ai->ai_family);
		result = 0;
		break;
	}
	freeaddrinfo(ai);
